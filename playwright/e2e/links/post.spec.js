...
test.describe("POST /api/links", () => {
  ...
  test("não deve encurtar quando a url original não for informada", async ({
    links
  }) => {
  ...
  test("não deve encurtar quando o título não for informado", async ({
    links
  }) => {
  ...
  test("não deve encurtar quando a url original é inválida", async ({
    ...
    const response = await links.createLink(
      { ...user.link, original_url: "teste@teste.com.br" },
      token,
    );
